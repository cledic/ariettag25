...
static int hcsr04_ovest_init(void)
{	
	int rtc;
	...
	if (class_register(&hcsr04_class)<0) goto fail_rc;
	...
	rtc = request_irq(gpio_irq, gpio_isr, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_DISABLED , "hc-sr04_ovest.trigger", NULL);
...
static void hcsr04_ovest_exit(void)
{
	if (gpio_irq!=-1) {	
...
module_init(hcsr04_ovest_init);
module_exit(hcsr04_ovest_exit);
